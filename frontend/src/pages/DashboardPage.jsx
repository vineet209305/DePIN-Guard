        ...
        <div className="data-section">
          <div className="section-header">
            ...
            <span className="sensor-count">
              {!loading && recentData.length > 0 ? `${recentData.length} readings` : ''}
            </span>
          </div>
            ...
            <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="bar-chart-svg">
              ...
              {[0, 25, 50, 75, 100].map((tick) => (
                <g key={tick}>
                  <line
                    x1={MARGIN.left}
                    y1={MARGIN.top + CHART_HEIGHT - (tick / 100) * CHART_HEIGHT}
                    x2={SVG_WIDTH - MARGIN.right}
                    y2={MARGIN.top + CHART_HEIGHT - (tick / 100) * CHART_HEIGHT}
                    stroke="rgba(255,255,255,0.06)"
                    strokeDasharray="4 4"
                  />
                  <text
                    x={MARGIN.left - 10}
                    y={MARGIN.top + CHART_HEIGHT - (tick / 100) * CHART_HEIGHT + 4}
                    fontSize="11" textAnchor="end" fill="#4b5563"
                  >
                    {tick}
                  </text>
                </g>
                ...
                return (
                  <g key={i}>
                    <rect
                      ...
                      fill="url(#barGradient)" rx="4" opacity="0.9"
                    />
                    {data.raw > 0 && (
                      <text
                    ...
                    <text
                      x={x + barWidth / 2} y={SVG_HEIGHT - 8}
                      fontSize="10" textAnchor="middle" fill="#4b5563"
                    >
                      {data.label}
                    </text>
                  </g>
